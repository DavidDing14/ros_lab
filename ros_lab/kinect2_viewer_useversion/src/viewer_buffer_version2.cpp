...
bool findImage(kinect2_viewer::UseStamp::Request &req, kinect2_viewer::UseStamp::Response &res)	//dxh
{
  ...
  ROS_INFO("request: stamp = %ld", reqtoSec);
  double minSec = abs(imageRGBD[0].stamp.toSec() - reqtoSec);
  int imageNo = 0;
  for (int i=1; i<imageRGBD.size(); ++i)
  {
...
private:
                ...
                const sensor_msgs::CameraInfo::ConstPtr cameraInfoColor, const sensor_msgs::CameraInfo::ConstPtr cameraInfoDepth)
  {
    ...
    ROS_INFO("new image coming and its stamp is %ld", imageRGBD[0].stamp.toSec());
  }
  ...
  void imageViewer()
  {
    ...
    for(; running && ros::ok();)
    {
	  ...
	  ROS_INFO("newData is %ld", imageRGBD[0].stamp.toSec());
        }
        else
        {
          save = true;
