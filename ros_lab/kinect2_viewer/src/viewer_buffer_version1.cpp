...
public:
  ...
  ros::Time stamp;
public:
  dataRGBD(pcl::PointCloud<pcl::PointXYZRGBA>::Ptr cloud, cv::Mat color, cv::Mat depth, cv::Mat depthDisp, ros::Time stamp)
  {
	this->cloud = cloud;
	this->color = color;
	this->depth = depth;
	this->depthDisp = depthDisp;
	this->stamp = stamp;
  }
  ~dataRGBD()
  {
...
{
  ROS_INFO("request: stamp = %ld", req.a.toSec());
  res.sum = 18763;
...
private:
                ...
                const sensor_msgs::CameraInfo::ConstPtr cameraInfoColor, const sensor_msgs::CameraInfo::ConstPtr cameraInfoDepth)
  {
    ...
    this->color = color;
    this->depth = depth;
    this->stamp = imageColor->header.stamp;
    updateImage = true;
    updateCloud = true;
    lock.unlock();
  }
  ...
  void imageViewer()
  {
	...
	stamp = this->stamp;
      ...
      dataRGBD newData(cloud, color, depth, depthDisp, stamp);	//dxh
      ...
      case 's':
        if(mode == IMAGE)
        {
          ...
          cv::imshow("test cloud", imageRGBD[0].color);
        }
        else
        {
          save = true;
